...
INuiSensor*		KinectView::m_pNuiSensor	= NULL;
bool			KinectView::sensorStarted	= false;



bool KinectView::getSensorStarted() {
...
HRESULT KinectView::InitDevice() {
	...
	//m_xyScale = tanf(NUI_CAMERA_DEPTH_NOMINAL_HORIZONTAL_FOV * DegreesToRadians * 0.5f) / (m_depthWidth * 0.1f);  
	m_xyScale = 0.0025f;
...
HRESULT KinectView::Render() {
	...
	XMMATRIX viewProjection = XMMatrixMultiply(XMMatrixIdentity(), m_projection);
